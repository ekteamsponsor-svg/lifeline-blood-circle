...
import { 
  ...
  Quote, BookOpen, ChevronRight, Sparkles, Star, ShieldAlert, Calendar, KeyRound
} from 'lucide-react';

// Firebase initialization
const firebaseConfig = {
  apiKey: "AIzaSyC8ReuMtfkQSqAAAup0Me5j79Q0et3SsxI",
  authDomain: "lifeline-blood-circle.firebaseapp.com",
  projectId: "lifeline-blood-circle",
  storageBucket: "lifeline-blood-circle.firebasestorage.app",
  messagingSenderId: "939453077057",
  appId: "1:939453077057:web:9d63a5bac02835b578d8c0",
  measurementId: "G-44PE5HWJST"
};

const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);
const appId = typeof __app_id !== 'undefined' ? __app_id : 'lifeline-blood-circle';
          ...
          <>
            ...
            {activeTab === 'guide' && (
              <div className="space-y-6 animate-[zoomIn_0.3s] pb-10">
                ...
                <div className="space-y-4">
                  {[
                    { title: "রক্তের দাতা খোঁজা", text: "প্রয়োজনীয় রক্তের গ্রুপ অনুযায়ী হোম পেজের বাটনগুলোতে ক্লিক করুন। সেখানে সকল যোগ্য দাতার তালিকা পাওয়া যাবে। আপনি চাইলে সার্চ বক্সে নাম বা ঠিকানা লিখেও দাতা খুঁজতে পারেন।" },
            ...
            {activeTab === 'register' && (
              <form onSubmit={handleRegister} className="space-y-4 animate-[zoomIn_0.2s] pb-10">
                <h2 className="text-2xl font-black text-red-600 mb-6">নতুন নিবন্ধন</h2>
                <div className="space-y-4">
                ...
                {isLoggedIn && currentUserData ? (
                  <form onSubmit={handleProfileUpdate} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button type="submit" disabled={isUpdating} className="bg-red-600 text-white p-5 rounded-[28px] font-black flex items-center justify-center gap-2">{isUpdating ? <Loader2 size={20} className="animate-spin"/> : <Save size={20}/>} সেভ করুন</button>
                    </div>
                  </form>
                ) : (
            ...
            {activeTab === 'memberList' && (
              <div className="space-y-4 animate-[zoomIn_0.2s]">
                <div className={`flex items-center gap-3 px-5 py-4 rounded-[28px] border ${darkMode ? 'bg-zinc-900 border-zinc-800 shadow-inner text-white' : 'bg-gray-50 border-gray-100 shadow-inner text-zinc-900'}`}>
                  <Search size={20} className="opacity-40" /><input type="text" placeholder="নাম, ঠিকানা বা ফোন..." className="bg-transparent outline-none w-full text-sm font-bold" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  {selectedGroup && (
                ...
                <div className="space-y-3">
                  {members.filter(m => {
                    ...
                    const matchesSearch = (m.name || '').toLowerCase().includes(q) || (m.address || '').toLowerCase().includes(q) || (m.phone || '').includes(q);
